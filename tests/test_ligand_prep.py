...
def test_ligand_prep_enriches_dock_pool_with_pdbqt_paths(monkeypatch, tmp_path: Path, repo_root: Path) -> None:
    cfg = load_workflow_config(repo_root / "configs" / "ci.yaml")
    paths = ensure_run_dirs(tmp_path)
    pool = pd.DataFrame(
        [
            {"compound_id": "c1", "hit_smiles": "CCO", "tanimoto_similarity": 0.9, "passed_tier_ab": True},
            {"compound_id": "c2", "hit_smiles": "CCC", "tanimoto_similarity": 0.8, "passed_tier_ab": True},
        ]
    )
    pool.to_parquet(paths["filters"] / "dock_pool.parquet", index=False)

    def fake_which(name: str):
        return "obabel" if name == "obabel" else None
    ...
    monkeypatch.setattr("workflow.steps.ligand_prep.shutil.which", fake_which)
